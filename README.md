...
- **Backend → AI/ML Engineer** | SSAFY (삼성 청년 SW 아카데미) AI 트랙 수료 중
- 도메인 특화 LLM 파인튜닝부터 프로덕션 서빙까지 **End-to-End** 경험
- **Interests**: `LLM Fine-tuning` · `RAG` · `vLLM` · `Multi-Agent Systems` · `Data Pipeline`
...
**50 Repositories** · **2,000+ Downloads** · End-to-End ML Pipeline

Qwen3 기반 도메인 특화 모델 v1→v9까지 9회 반복 파인튜닝.
31,625건 SFT 데이터 직접 구축 (사주 · 점성술 · 타로 · 화투).
DPO/SimPO 선호 정렬 + AWQ/GGUF/MLX 양자화로 프로덕션 서빙.

```
Data Collection → Preprocessing → SFT (v1~v9) → DPO/SimPO → Quantization → vLLM Serving
```

| Category | Count | Highlights |
|----------|-------|-----------|
| **Models** | 20 | Qwen3 4B/8B, RSLoRA, AWQ/GGUF/MLX |
| **Datasets** | 28 | 31.6K SFT samples, 100K+ 중한 번역 코퍼스 |
| **Spaces** | 2 | Gateway, Demo |

**Speaki** — AI 영어회화 튜터 (SFT + DPO + SimPO 3단계 정렬 학습)
